...
import { ArrowLeft, Gift, Download, Zap, ShoppingCart } from "lucide-react";
...
const Quotation = () => {
  ...
  // Support both single config and cart items
  ...
  // Prepare items for quotation (either from cart or single config)
  ...
  // Calculate competitor prices once when component mounts or total price changes
  useEffect(() => {
    if (totalPrice > 0) {
  ...
  const handleWheelResult = (prize: string) => {
    ...
    setShowWheel(false);
    incrementRealQuotation(); // Increment counter when quotation is completed
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      await generateQuotationPDF(quotationItems, phoneNumber, wheelResult, competitorPrices);
      toast({
        title: "PDF Gerado!",
        description: "Seu orçamento foi baixado com sucesso.",
      });
    } catch (error) {
      toast({
              ...
              {!showWheel && !wheelResult ? (
                <div className="space-y-6">
                  ...
                  <div className="text-xs text-muted-foreground text-center">
                    Prêmios: Descontos de 2%, 4%, 6%, 10%, Power Bank, Squeeze, Copo, Bag, Caderno
                  </div>
                </div>
              ) : showWheel ? (
                ...
                <div className="space-y-6 text-center">
                  ...
                  <div className="p-4 bg-success/10 border border-success/30 rounded-lg">
                    <p className="text-sm text-foreground font-medium">
                      ...
                      Fale com nosso representante para garantir sua condição especial!
                    </p>
                  </div>
                </div>
              )}
